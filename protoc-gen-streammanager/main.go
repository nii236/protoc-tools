...
func generateStreamManagerBase(g *protogen.GeneratedFile, file *protogen.File) {
	...
	g.P("	sendMu    sync.Mutex // protects Stream.Send from concurrent calls")
	g.P("}")
	g.P()
	...
	g.P("}")
	g.P()
	g.P("func (s *StreamManager[TReq, TResp]) SendToStream(sessionID string, response *TResp) (err error) {")
	g.P("	s.mu.RLock()")
	...
	g.P("			s.RemoveStream(sessionID)")
	g.P("			err = fmt.Errorf(\"stream %s panicked: %v\", sessionID, r)")
	g.P("		}")
	g.P("	}()")
	g.P("	")
	g.P("	ctx.sendMu.Lock()")
	g.P("	err = ctx.Stream.Send(response)")
	g.P("	ctx.sendMu.Unlock()")
	g.P("	if err != nil {")
	g.P("		s.RemoveStream(sessionID)")
	g.P("	}")
	g.P("	return err")
	...
	g.P("	var deadSessions []string")
	g.P("	for _, ctx := range contexts {")
	g.P("		func() {")
	g.P("			defer func() {")
	g.P("				if r := recover(); r != nil {")
	g.P("					// Stream panicked (likely disconnected), mark for removal")
	g.P("					deadSessions = append(deadSessions, ctx.SessionID)")
	g.P("					lastErr = fmt.Errorf(\"stream %s panicked: %v\", ctx.SessionID, r)")
	g.P("				}")
	g.P("			}()")
	g.P("			ctx.sendMu.Lock()")
	g.P("			err := ctx.Stream.Send(response)")
	...
	g.P("	}")
	g.P()
	g.P("	// Clean up dead streams")
	g.P("	for _, sessionID := range deadSessions {")
	g.P("		s.RemoveStream(sessionID)")
	g.P("	}")
	g.P()
